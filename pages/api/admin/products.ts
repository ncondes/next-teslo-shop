...
const updateProduct = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
   ...
   // TODO: localhost:3000/products/img.jpg
   ...
   try {
      ...
      if (!product) {
         await db.disconnect();
         return res.status(400).json({ message: 'There are no products with the id provided.' });
      }
      ...
      console.error({ error });
      await db.disconnect();
      return res.status(500).json({ message: 'Check server console.' });
   }
};
